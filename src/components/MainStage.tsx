...
const MainStage = (props: MainStageProps) => {
  ...
  const randomDogBreedGenerator = useCallback(() => {
    ...
    const totalWeightedBreeds =
      ...
      Object.keys(hardBreeds).length *
        (props.currentLevel === "easy" ? 1 - proportionOfEasyBreeds : 0.8);
  ...
  const updateProportions = () => {
    if (curScore > 0 && curScore % 85 == 0)
      setProportionOfEasyBreeds(Math.max(0, proportionOfEasyBreeds - 0.2));
  };
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      for (const breed of breedsToShow) {
        fetchDogImg(dogBreedsToQuery[breed as keyof typeof dogBreedsToQuery])
          .then((response) => {
